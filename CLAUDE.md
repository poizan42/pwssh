...
**`exec` and `shell` both work end to end over WinRM.** `ssh pwssh-test whoami` returns the remote's `DOMAIN\user`, and `ssh pwssh-test` gives a cmd.exe session — with a real terminal when the client asks for one. The suite runs 62–70 cases per transport: **WinRM is 70/70**, loopback **61/62** with one environmental exception — the pty case that this client machine's ConPTY breaks (see *Running and testing*). The two runs differ in composition rather than count — WinRM has the graceful-degradation and IPv6 cases plus the gateway-ports check; loopback has the wrong-username check that the WinRM alias takes from `ssh_config`, along with the reverse-forward release and bind-failure cases that need the far side to be this machine.
...
- **Seen once and never reproduced: the client→agent SFTP stream lost 4 bytes of alignment during a valve trip.** The agent reported `bad SFTP packet length 83886080` and the transfer came back with a wrong hash. 83886080 is `0x05000000`, i.e. a `READ` type byte (5) read where a length prefix belonged, so the stream was off by exactly the 4-byte prefix. It has not recurred in 12 attempts: 4 on the dev host, 4 with `-LatencyMs 300` so that reads genuinely park (`parked=1`, which is what makes `ReplayParked` inject into that stream — the obvious suspect), 3 targeted over WinRM with `PWSSH_SFTP_FAULT_AFTER_KIB`, and a full WinRM suite run that passed 70/70. Recorded rather than dismissed: the signature is specific enough to be worth recognising if it returns, and the trip path — which injects replayed reads into a stream it does not otherwise write to — is where to look first. It is not known to be a regression from any particular change.
