...
`tests/Pwssh.Tests` exists because the PowerShell suite is driven entirely through the stock OpenSSH client, and a client cannot ask for what it has no command for. **32 tests, ~35 s**, `dotnet test tests/Pwssh.Tests`. It needs no dev host and no DLL — it compiles the product sources in and hosts the engine itself.
